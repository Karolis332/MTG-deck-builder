...
- **T10:** 60-card "Rebuild with model" was unusable (commander heuristics, no meta). (A) DONE 2026-09-08 (1a9a3a3): `POST /api/decks/:id/optimize-apply` saves main − cuts + paired adds as an "(Optimized)" deck; header button reads "Optimize with model" for 60-card formats. Next (B) a meta-archetype builder: nearest `archetype_win_stats`/`community_decks` archetype by colour + card overlap → consensus list (avg copies from `meta_card_stats`), collection-biased. Data: VPS pipeline now scrapes Standard every cycle incl. mtgo.com W/L; pull with `py -3.13 scripts/sync_vps_meta.py`. Follow-ups: re-run `aggregate_community_meta.py` after the mtgo backfill so `placement_weighted_score` uses real records; MTGTop8 deep history needs a different listing (the `cp=` param does not page); MTGGoldfish deck pages need a browser (Cloudflare managed challenge); replace the colour-guild archetype label on mtgo lists with a nearest-MTGTop8-archetype classifier.
