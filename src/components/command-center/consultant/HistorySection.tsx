    ...
    <section className="hud-panel">
      ...
      {!collapsed && (
        <div className="space-y-1 px-3 pb-3">
          ...
          {items.map((item) => (
            <div key={item.key} className="flex items-center gap-2 rounded-lg px-2 py-1 text-sm">
              <span className="flex-1 truncate">{item.label}</span>
              <span className="shrink-0 text-[11px] text-muted-foreground">{item.detail}</span>
              <span className="shrink-0 text-[11px] text-muted-foreground">{relativeTime(item.at)}</span>
              {item.restore && (
                <button
