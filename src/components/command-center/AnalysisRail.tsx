      ...
      {showExplanation && explanation && (
        <div className="mb-3 rounded-xl border border-primary/30 bg-primary/5 p-3 animate-slide-up">
          <div className="mb-2 flex items-center justify-between">
            ...
            <button onClick={onDismissExplanation} className="text-xs text-muted-foreground hover:text-foreground">
              Dismiss
            </button>
          </div>

          <p className="mb-2 text-sm text-foreground/80">{explanation.strategy}</p>

          {explanation.themes && (
            <div className="mb-2 flex flex-wrap gap-1">
              {explanation.themes.split(', ').map((t) => (
                <span key={t} className="rounded-full bg-accent px-2 py-0.5 text-xs font-medium">
                  {t}
                </span>
              ))}
            </div>
          ...
          <div className="space-y-1.5">
            {Object.entries(explanation.roleBreakdown).map(([role, cards]) => (
              <details key={role} className="group">
                <summary className="flex cursor-pointer items-center gap-1.5 text-sm font-medium text-foreground/70 hover:text-foreground">
                  <span className="transition-transform group-open:rotate-90">&#9654;</span>
                  {role} ({cards.length})
                </summary>
                <div className="ml-4 mt-1 space-y-0.5">
                  {cards.map((cardName) => (
                    <div key={cardName} className="flex items-start gap-1 text-xs">
      ...
      {isCommanderFormat && (
        <div className="mb-4">
          ...
          >
            ...
            <span className={cn('text-sm text-muted-foreground transition-transform', showCombos && 'rotate-180')}>&#9660;</span>
          </button>

          {showCombos && (
            <div className="mt-2 space-y-3 rounded-xl border border-border bg-card p-3 animate-slide-up">
              <div className="flex items-center justify-between">
                <span className="text-sm text-muted-foreground">Powered by Commander Spellbook</span>
                <button
                  onClick={onRescanCombos}
                  disabled={combosLoading}
                  className="rounded-md bg-accent px-2.5 py-1 text-xs font-medium text-accent-foreground transition-colors hover:bg-accent/80 disabled:opacity-50"
                >
              ...
              {combosLoading && (
                <div className="flex items-center justify-center py-4">
                  ...
                  <span className="ml-2 text-sm text-muted-foreground">Analyzing deck for combos...</span>
                </div>
              )}
              ...
              {includedCombos.length > 0 && (
                <div>
                  <h4 className="mb-1.5 text-sm font-semibold text-green-400">
                      ...
                      <details key={combo.id} className="group rounded-lg border border-green-500/20 bg-green-500/5 p-2">
                        <summary className="flex cursor-pointer items-center gap-2 text-sm">
                          ...
                          {combo.popularity != null && combo.popularity > 0 && (
                            <span className="shrink-0 text-[11px] text-muted-foreground">pop: {combo.popularity}</span>
                          )}
                        </summary>
                        <div className="mt-2 space-y-1 pl-4">
                          {combo.description && <p className="text-xs text-foreground/80">{combo.description}</p>}
                          {combo.prerequisites && (
                          ...
                          {combo.results.length > 0 && (
                            <div className="flex flex-wrap gap-1">
                              {combo.results.map((r) => (
                                <span key={r.feature_name} className="rounded-full bg-primary/10 px-1.5 py-0.5 text-[11px] font-medium text-primary">
                                  {r.feature_name}
                                </span>
                              ))}
                            </div>
                ...
                return (
                  <details className="group/near">
                    <summary className="flex cursor-pointer items-center gap-2 text-sm">
                      ...
                      <h4 className="text-sm font-semibold text-amber-400">Near-Miss Combos ({almostIncludedCombos.length})</h4>
                      {includedCombos.length === 0 && (
                        <span className="text-xs text-muted-foreground">— check these for potential additions</span>
                      )}
                    </summary>
                    <div className="mt-2 space-y-2">
                      {sorted.slice(0, 15).map((combo) => (
                        <details key={combo.id} className="group rounded-lg border border-amber-500/20 bg-amber-500/5 p-2">
                          <summary className="flex cursor-pointer items-center gap-2 text-sm">
                            ...
                            <span className="shrink-0 rounded-full bg-amber-500/20 px-1.5 py-0.5 text-[11px] font-bold text-amber-400">
                          ...
                          <div className="mt-2 space-y-1 pl-4">
                            ...
                            {combo.results.length > 0 && (
                              <div className="flex flex-wrap gap-1">
                                {combo.results.map((r) => (
                                  <span key={r.feature_name} className="rounded-full bg-amber-500/10 px-1.5 py-0.5 text-[11px] font-medium text-amber-400">
                                    {r.feature_name}
                                  </span>
                                ))}
                              </div>
                      ...
                      {sorted.length > 15 && (
                        <p className="text-center text-xs text-muted-foreground">+{sorted.length - 15} more near-miss combos</p>
                      )}
                    </div>
                  </details>
                );
