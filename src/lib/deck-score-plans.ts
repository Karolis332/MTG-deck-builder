...
export const PLAN_RECIPES: readonly PlanRecipe[] = [
  ...
  },
  {
    ...
    roles: [
      { key: 'stabilisation', essential: true, min: 7, max: 12, cmd: { min: 4, max: 9 }, brawl: { min: 4, max: 13 }, deadline: 3, fills: answer(3) },
      { key: 'engine', essential: true, min: 12, max: 19, cmd: { min: 7, max: 17 }, brawl: { min: 8, max: 17 }, deadline: 4, fills: (f) => f.isDrawEngine || velocity(4)(f) },
      { key: 'finisher', essential: true, min: 3, max: 8, cmd: { min: 3, max: 9 }, brawl: { min: 3, max: 9 }, deadline: 7, fills: threat(4, 7, 0.6) },
