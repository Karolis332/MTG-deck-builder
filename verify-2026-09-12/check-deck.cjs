/**
 * Brawl decklist validator against the live desktop DB + the user's Arena collection.
 *
 *   node verify-2026-09-12/check-deck.cjs <list.txt> [--ci BR]
 *
...
for (const { qty, name } of lines) {
  ...
  if (leg.brawl !== 'legal') errors.push(`NOT BRAWL LEGAL (${leg.brawl || '?'}): ${c.name}`);
