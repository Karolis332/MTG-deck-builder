...
const DECKS: Deck[] = [
  ...
  { key: 'cabbage', title: 'The Cabbage Merchant', file: 'the-cabbage-merchant.txt', commander: 'The Cabbage Merchant', fixedSwaps: 'cabbage-merchant-upgrade.txt',
    caveat: 'Paper Commander upgrade v2 (2026-09-18), owned cards only, Transmutation Font included; five cards on order (Trail of Crumbs, Beast Within, Feasting Hobbit, Feasting Troll King, Quina) are a separate arrival plan in the guide, not in this list. Food engine for a 4-player pod: Took + Manufactor + Second Harvest multiply tokens, Jaheira / Night of the Sweets\' Revenge / the commander turn them into mana, Halsin / Trudge Garden / Displaced Dinosaurs / Bumi\'s Feast Lecture turn them into damage. No infinite loop; one opponent per combat. Guide with win conditions, engine rules and keep/mull: proposals/cabbage-merchant-upgrade.md. Two adds are also sleeved elsewhere as assumed second copies (Llanowar Elves, Eternal Witness).' },
];
...
<div class="swaps">${plan.swaps.map((sw, i) => `<label class="swap" data-key="${d.key}:${esc(sw.out.name)}>${esc(sw.in.name)}"><input type="checkbox"><span class="n">${i + 1}</span>${tile(sw.out, sw.qty)}<span class="arrow">→</span>${tile(sw.in, sw.qty)}<p class="why">${esc(sw.why)}</p></label>`).join('')}</div></section>`;
  ...
  const html = `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Deck edit plans — ${new Date().toISOString().slice(0, 10)}</title><style>${css}</style></head><body>`
    + `<header><h1>Deck edit plans — final recommendation (2026-09-18)</h1><nav>${DECKS.map((d) => `<a href="#${d.key}">${esc(d.title.split(',')[0])}</a>`).join('')}</nav><span id="done"></span><span style="margin-left:auto;font-size:12px;color:#b5a27b;max-width:46%">${esc(legend)}</span></header>`
