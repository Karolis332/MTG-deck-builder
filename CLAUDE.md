...
### 2026-09-20 — Deck Score v1.3 stages 1–4a committed (not deployed); stage 4b running
...
- **Open:** e5ac457 stage 4a — ONE joint Commander floor `.574` (p95 of max Q over all 11 recipes on 1,000 stride controls; replaces .542/.559), bands re-measured on a commander-disjoint stride, Thousand-Year Storm/Ponder parsed: fresh stride controls 197/200 (target 190 ✅), validation 200/200, cEDH 88/90, gaming 3/3, anchors 14/16 (tazri-upgraded 32 vs 45–65, Vivi 64 vs 70–85 — both Brawl decks graded on Commander bands), closing reads 1/200 (one-card alt-win pool; `combo` still on the .30 floor). Perf fix found while chasing a false gate claim: `deck-gate-parse.ts` `resolveLines` ran an unindexable `LIKE '// %'` scan per name even after an indexed legal exact hit (54 ms → 0.03 ms/name; fixture load 4.5 s → 31 ms; suite 1140 tests in 8.6 s) — the 16 'failures' were 15 s timeouts under load. Brawl cohort exists: CF corpus `historicBrawl` = 11,281 100-card Arena Brawl decks; sample `scripts/brawl-sample.sql` → `verify-2026-09-20/brawl-sample.csv` (1,746 lists, untracked). Running: stage 4b (brief `…-stage4b-…md`): Brawl bands + Brawl joint floor from that sample, closing floor, tazri-upgraded party read, all five Brawl fixtures re-graded — anchors ≥ 15/16, Brawl fresh controls ≥ 190/190; then 4c control finisher schedule (`…-stage4-…md`), Opus refuter, deploy, freeze 1.3.0. Standard total needs the §7 interaction/advantage norms revisited — operator's five spec questions still unanswered.
