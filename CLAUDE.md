...
### 2026-09-21 — Deck Score v1.3 stages 1–4b committed (not deployed); stage 4c running
...
- **Open:** 579f2c1 stage 4b — Brawl profile measured from the Historic Brawl corpus (`scripts/brawl-sample.sql` → `verify-2026-09-20/brawl-sample.csv`, 1,746 lists / 300 commanders, stride 1,166 train / 580 holdout): Brawl bands for the 7 recipes with ≥ 30-list cohorts, `Q_BASELINE_JOINT_BRAWL = .675`, `Q_BASELINE_CLOSING = .323` (one-card alt-win pools are real lines — 28/28 cEDH positives close on one), Tazri party reveal typed. Anchors **15/16** (Vivi 76 IN; tazri-upgraded 20 OUT — typal Q .655 under the .675 floor, 10 uncovered copies), Commander fresh 197/200 closing 0/200, validation 200/200, Brawl fresh 200/200, cEDH 88/90, 1158 tests. Perf fix from 4a: `deck-gate-parse.ts` skipped the per-name unindexed `LIKE '// %'` scan (fixture load 4.5 s → 31 ms; suite 8.6 s). Running: stage 4c (brief `deck-score-v13-stage4c-2026-09-21.md`): S saturation measured per profile (Brawl window .675→.70 pins every real deck at S 100; Commander `.70` stays — a move is an operator/astra decision), Brawl closing floor, tazri coverage, `SCORE_VERSION 1.3.0-rc2`; then Opus refuter over stages 1–4c → deploy build-api (DEPLOY §5) → control finisher schedule stage (`…-stage4-…md`) → freeze 1.3.0. Standard total needs the §7 interaction/advantage norms revisited — operator's five spec questions still unanswered.
