...
### 2026-09-22 — Deck Score v1.4: stages 0–3d committed (rc4, anchors 14/16); §10.10 ledger-as-choice committed; rc6 tree (3e+3g+3h) uncommitted and red; stage 3i running
...
- **Open:** d70bc89 stage 1d (§10.8: rituals paid once, deployment from the shared per-turn ledger with atomic affordability, finite cantrip ledger, Commander H20 with decay, finish 3×40 per opponent, zero-output bodies no longer raise access K; 1354 tests, bands verify 0, domain `v14-stage1d`) — acceptance FAIL: eligible Commander W=0 74/1460 (5.1 %, target ≤73), W p50 23.1 (37.5), piles 233/330 per 1000 <25 (≥950), anchors 9/16 (Meren 56, precon 23, Kuja 56, Cabbage paper 54 all LOW via the W cap; Vivi witness resolved at 86). PASS: cEDH H12↔H20 0/30 changes, p50 92, gaming 3/3, ~12 ms. Central finding: H12 separates piles (890–919/1000) but leaves 60 % of real lists at W=0; H20 saves real lists and lifts piles equally — piles and casual decks are indistinguishable by kill schedule or useful mass. 109ed2c spec §10.9 (astra, rc 75 but delivered): headline = percentile of the composed absolute total in a per-profile frozen family-weighted empirical CDF (midrank ties, no interpolation), components + absolute total stay diagnostics, H20 and the `20+.8·min` cap kept for every profile, the ≥95 %-of-piles-<25 gate and the W p10/p50/p90 floors RETIRED for a top-tail guard (≤100/1000 ranks ≥95 per construction/profile, ≤20/200 retained), Δrank ≤ 1 on no-benefit probes tested explicitly, 16 anchors restated as rank bands (precon p15–40 mandatory, Meren p65–90, both Cabbages p50–90, Balloon Con p95–100), build order 2c reference → 3 adversarial → 4 holdouts → 5 refuter → 6 UI (rank headline + absolute diagnostics). 9ae7025 stage 2b: `freeze --write` moved 0 constants (S norms already matched the 1d domain 21fa0be…), markers replaced. c2a755c stage 2c: unrounded `absoluteTotal` (`score` = its rounding, unchanged), `src/lib/deck-score-reference.ts` + frozen knots `deck-score-reference/{commander,brawl}.json` (1,440 rows/182 families; 785/179; midrank ties, weighted mean 50 to 1e-9, byte-stable double freeze), additive payload `headline/rank/rankDisplay/reference/evidence`, `bands reference freeze|verify` inside `bands verify`; training rank p10/p50/p90 cmd 10.9/51.7/89.9, brawl 9.3/47.6/90.1; cEDH rank p10 96.5 / p50 99.7 PASS; top-tail 0/0/5/1 of 200 PASS; anchors 10/16 FAIL (#8 ramos ranks 0.00 — `deck-validation` warns on undersized; tazri-beacon p79.9 vs p35–75; Kuja p8.3 vs p25–60; Standard blocked at 27 date-families); gaming residue now in rank: delete-off-plan Δrank 16 cmd / 76 brawl at ΔS 0 (OPEN); p50 7.7 ms; 1392 tests. d06db7e stage 3: scorer-owned undersized gate (`deck-score-gates.ts`: below 99/98 → size fail, 19 cap, no rank — `deck-validation` still only warns for the editor), M/C/A densities over `max(N0,N)` (submitted-N defect), rules-failed rows leave norm estimation, Standard reference frozen on the 42 event families; `SCORE_VERSION 1.4.0-rc1`, references re-cut (cmd 1,429/389 knots, brawl 783, standard 270/42), 0 S constants moved; delete-off-plan residue 0.00 at every k; top-tail guard 8/8 PASS (fresh controls 845/516 — holdout cohort yields < 1,000 legal piles); anchors 13/16 (out: tazri-beacon p79.8 vs p35–75, Kuja p8.3 vs p25–60, #13 competitivebrawl has no cohort so ≥14/16 needs both to move — a fit is forbidden, so the anchor gate is structurally FAIL until a competitive-Brawl reference exists); OPEN residue: replace-off-plan-unknown k=1 gains on 3 cmd / 5 brawl lists (W utilisation rises when a consumer blanks; mean/shape estimators move); in-suite latency pin → 60 ms sanity bound (isolated p50 7.7 ms); 1414 tests. 35aa500 stage 3b: unresolved slots imputed pessimistically at one seam (`blankFeature`: spell at max(7, deck max MV), colour 0, worst curve bin; M/C/W under spell/land/mdfc hypotheses, worst kept), recipe demands no longer clamped to the identified pool; `1.4.0-rc2`; Brawl unknown-slot residue closed (5 → 0 lists), Commander 3 → 1; root-caused OPEN: `requiredCopies()` sizes a line from the mean of the r cheapest sources (non-monotone); NEW: Standard is free to pad (add 10 filler cards above 60 → ΔT_abs 19, Δrank 39 — no maximum size, access not diluted by N) and a 60 → 59 trim is not size-failed; 1432 tests. eb9e575 stages 3c+3d: land surplus vs N0 (padding a flooded Standard list bought +42 landFit), size gate for every profile (26 sub-60 Standard scrapes leave the reference), `bestLine` max over output-level subsets, and the ROOT CAUSE of both W monotonicity defects — `scheduleDamage` tested affordability against the residual EXPECTED budget, so a hair of dilution or a blanked pump flipped a whole deployed copy and a whole turn; now `affordable()` = chance the leftover covers one copy (0 below 75 % of cost, linear ramp, `AFFORD_BAND .25` — a modelling constant, two alternatives rejected in `v14-stage3d-report.md`); `1.4.0-rc4`, scheduler `v14-stage3d`, references re-cut (cmd T_abs p50 36.6 → 49.6, 399 knots); anchors **14/16** (mandatory all PASS; out: Kuja p17 vs p25–60, #13 uncalibrated), cEDH p50 91.5, delete/blank property 0 violators cmd+brawl; OPEN: Standard `add-unknown-slots` k=1 7/499 gain (deployment greedy's fixed in-turn order); 1461 tests. Backup of the 3c tree: `~/.claude/harness/runs/deck-score-2026-09-19/stage3c-wip.patch`. Stage 3e (UNCOMMITTED, backup `~/.claude/harness/runs/deck-score-2026-09-19/stage3e-wip.patch`, `rc5`): per-turn deployment = exact fractional knapsack against the cap-independent budget — Standard `add-unknown-slots` k=1 7 → 0; but the optimum spends mana the old order stranded, W rises broadly, anchors 14/16 → 11/16 (mandatory all PASS; tazri-beacon p80 OUT HIGH, tazri-upgraded p28 LOW, Standard #15 p61 / #16 p50 LOW, Kuja now PASS), and a THIRD channel is exposed: `deployBudget = produced − spellSpend − upkeepSpend` with MANDATORY expected-value spell/upkeep debits, so deletion or dilution frees deployment mana (Commander `add-unknown-slots` 5/8+/8+ violating, Δrank 38; Brawl 1). Rule emerging: any mandatory debit computed from an expectation is a non-monotone channel — every ledger debit must be a choice. 8de5767 spec §10.10 (astra): every discretionary ledger debit is a choice, compulsory = rule consequences only, affordability boxes frozen at branch-entry mana, common currency = discounted damage through H, finish-first objective, actual-N dilution + `AFFORD_BAND .25` kept, anchor bands UNCHANGED (rc5 11/16 = FAIL, release blocked on honest misses). Uncommitted rc6 tree (backups `stage3{e,g,h}-wip.patch` in the runs dir; `v14-stage3g-report.md` has phases 1/2 + Stage 3h): one per-turn allocation replaces the four mandatory prepasses (ramp, shared spell, anthem, activations), blank slots never count as casts, `bestLine` ranks by the same `poolsAccess` the evaluator reports, blank cost as a hypothesis dimension; results: Standard k=1 0 violators, Commander 3 / Brawl 2 (residual = `u` short-circuits at t* while §1 says max over t ≥ t*; `colorFit` seam on one MDFC blank), anchors 11/16 (mandatory 8/8), vitest 38 failed untriaged, `bands verify` drifted RED on the brawl reference after the probe pass. Running: stage 3i debugger (`deck-score-v14-stage3i-2026-09-22.md`: explain the drift, `u = max_{t≥t*}`, colorFit, triage the 38, rc7 freeze). Then 3f measurement, stage 4 holdouts, refuter, build-api deploy + `unresolved` seam, stage 6 UI, app-sync, beta.4 (briefs written). Web: consultant markdown + null-history fix 935c71b deployed (black-grimoire-web log). Operator decision pending: the four non-mandatory anchor hypotheses (tazri-beacon, tazri-upgraded, Standard #15/#16) fail under the honest scheduler — §10.10 keeps the bands; only a declared decision with independent cohorts can change them. Also: signing, `OPENAI_*_MODEL`, spec Q1–5, competitive-Brawl cohort for #13.
...
### 2026-09-22 — Builder: conditional any-colour lands, price cap / deck budget / bracket target, fetch colour fit → build-api hotfix deployed; deck-score stages 3l–3q on the uncommitted tree
- **Did:** 87859e9 `isConditionalColoredProducer` flags costed (`{N},{T}`) and X-scaled any-colour activations (Conduit Pylons, Baldur's Gate were unconditional WUBRG fixers inside every ≥ 2-colour collection build's non-basic quota) + `alt_zone_cast` synergy category (Spider-Man 2099 → spellslinger, not aggro); afdd4da `BuildOptions.maxCardPrice/maxDeckPrice/bracket` (pool cap, `applyDeckBudget` same-role swap loop, bracket pool exclusions + `applyBracketTarget`, `price`/`bracket` response objects, `/build` 400 with field name), fetch lands matching one colour of a multicolour deck take the 0.4× discount; 41 tests. **Deployed as branch `hotfix/build-api-2026-09-22` = d6e0e2d + those two commits** (built in the `mtg-score-tmp` worktree: tsc 0, 74 tests) via DEPLOY §5; live probe through the site: Rakdos build $149.01 under a $150 budget, bracket 2 → 2, 0 off-colour fetches. Web 059cc2b deployed (form + result). Deck Score on the uncommitted tree: 3l (finish-first τ-scan, reserved-payment ledger, worst-case plan under reserved slots), 3m (unbounded rituals were persistent mana sources), 3n (unsatisfiable pool read as access 1; greedy backup pair), 3k rc8 freeze (`bands verify` MATCH ×2, 0 constants moved), 3o (hold bounded by the payment on offer) — property scan 0 real violators on 3 profiles × 2 arms; 3q running on the last probe channel (`upkeepAt` expected-spend debit entering the value order). Reports `~/.claude/harness/runs/deck-score-2026-09-19/v14-stage3{l,m,n,k,o}-report.md`, `~/.claude/harness/runs/builder-*-2026-09-22.md`.
- **Why:** the deck-score tree is uncommitted (rc8, 23 stale snapshot pins), so the builder fixes shipped on a hotfix branch off the deployed base instead of deploying an unrefuted intermediate score; a "must-include" exemption was skipped (no such concept exists in the builder); `build-hints` free-text budget parsing deferred (`budgetPerCard` there is a soft penalty, not a cap).
- **Open:** 3q → 3p (rc9 freeze + every stale pin re-pinned, suite green) → commit → 3f ∥ stage 4 → refuter → build-api deploy from `auto-improve` (supersedes the hotfix) → stage 6 UI → beta.4. Running: `/alternatives` + `priceUsd` engine unit, web deck workspace (per-card price, "find cheaper", consultant edits applied in place). Orphaned `find.exe` PIDs 10572/276 from another session burn two cores (operator to kill). Anchors 10/16 decision open; latency p50 ≈ 21 ms vs the 20 ms gate.

### 2026-09-22 — build-api /alternatives + priceUsd/owned on /build; web deck workspace live; 3r/3p state
- **Did:** b384d30 `src/lib/card-alternatives.ts` + `POST /alternatives` (local synergy/edhrec ranking, `land-intelligence` for lands, colour identity + singleton + `maxPrice`, `owned`, 61 ms in-process), `priceUsd` on `/build` cards; ba27f8e `owned` on `/build` cards only when a collection was supplied, `commander` string|array; 94f8dda owned tests mock `autoBuildDeck` (the real-build version timed out at 60 s in the full suite). All cherry-picked onto `hotfix/build-api-2026-09-22` (tip on the VPS) — live probe through the site: alternatives for City of Brass under $5 → Rocky Tar Pit / panoramas. Web 9f0e9c1 (workspace) deployed after two Opus refuter rounds (`~/.claude/harness/runs/web-workspace-refute-2026-09-22.md`: 11 findings fixed, 3 follow-ups fixed). Deck Score: 3r closed `381147684` k=1 (cast damage credited only for bought casts) but opened k=5 `380158752` +21.84 T_abs and +0.16 on `381362138`; 3p froze **rc9** (`bands verify` MATCH ×2, 0 constants moved, 24 pins re-pinned, anchors 10/16 mandatory 8/8, cEDH p10 96.0 / p50 99.7, top-tail 8/8 PASS) and left 2 behavioural STOPs (`v12` producer-on-commander W = 0, `stage1c` pinger no longer builds the spells line) and **latency p50 45.6 ms** (was 21). Backup `stage3p-wip.patch`; 3s running (latency exact caches → k=5 regression → the two STOPs → +0.16).
- **Why:** `/alternatives` ranks locally (no EDHREC/CF call) to stay under 300 ms; `owned` absent (not false) without a collection so the web charges full price only when the engine knows nothing.
- **Open:** after 3s: re-freeze rc10 (3p procedure, mostly no-ops) → commit the deck-score tree → 3f ∥ stage 4 → refuter → deploy `auto-improve` build-api (supersedes the hotfix) → stage 6 UI → beta.4. Web FAQ bracket sentence stale. Orphaned `find.exe` 10572/276 still burning two cores.
