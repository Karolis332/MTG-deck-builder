...
### 2026-09-21 — Deck Score: coverage round 1 committed → S design defect exposed; discriminant study running
...
- **Open:** 17697ce coverage round 1 — catalogue 4,453 → 14,830 known (generator over every corpus-played card + 91 atoms ≥ 3 cards each, `scripts/deck-score-atoms.ts` audit, 10.9 MB shard, 150 ms parse), real-list coverage p50 33 → 76 % / 37 → 75 %; refuter R1–R3 closed (`bands real` through `scoreDeckSafely`, `bands verify` re-measures 5 constants + 45 band cells, fixture commanders out of the strides); every constant re-frozen: joint floors **.683 / .733**, Brawl saturation .758, Commander closing floor unmeasurable (0 lines). **Result: S collapses** — with `Q_sat` spec-frozen at .70 the Commander window is .017 wide, Brawl's b breaks §9.2's b < .70, real lists at S = 0 85.5 % / 94 %, anchors 11/16. Cause (round-1 diagnosis): a fully typed catalogue types the control piles' draw pool too, so on-plan role-supply share no longer separates a deck from a pile; controls are also drawn to .93 coverage vs real p50 .76. R4 (two cEDH lists at 20, 92.9 % typed) = unmodelled tutor-assembled creature-combo family for `deck-score-win.ts`. Running: S discriminant study (`deck-score-s-discriminant-study-2026-09-21.md`, Opus, read-only + one script): nine candidate statistics (coverage-normalised Q, plan concentration, producer–consumer linkage, commander linkage, role-vector shape, synergy, curve/colour, combinations) on real training lists vs .93-matched AND real-coverage-matched controls, window = real p50 − control p95, AUC, anchors under each → then astra/§10 design → implement → refuter → deploy → app-sync (`deck-score-app-sync-verify-2026-09-21.md`) → desktop beta.4. Build-api stays on d6e0e2d (v1.2). Operator: tazri-upgraded band ruling (now 68 IN under round 1), signing path, five spec questions.
