...
### 2026-09-21 — Deck Score v1.3 stages 1–4c committed (not deployed); refuter + coverage sizing running
...
- **Open:** 61b8b66 stage 4c — saturation measured per profile (Commander `.70` = p80 of real lists, kept; `Q_SATURATION_BRAWL = .712` = Brawl p80, S=100 share 100 % → 12 %), `Q_BASELINE_CLOSING_BRAWL = .338`, four generator atoms, `SCORE_VERSION 1.3.0-rc2`; anchors 15/16 (tazri-upgraded 20: typal enabler is served by payoff 8:1 and the deck has two payoffs — band ruling needed, not a floor move), Commander fresh 197/200 closing 0/200, validation 200/200, Brawl fresh 200/200, cEDH 88/90, 1170 tests. **Finding that gates the product, not the maths:** real corpus lists have typed coverage median .34 (Commander) / .37 (Brawl) vs the .93 the control piles are drawn to, so under the live gate the median real deck reads S = 0 — the same defect behind the operator's ~57 %-coverage web builds. Running: Opus refuter over stages 1–4c (`deck-score-v13-refute-2026-09-21.md`: reproduce every frozen constant, cohort hygiene, six gaming probes, parser diff = 0) and a coverage-sizing measurement (`deck-score-coverage-sizing-2026-09-21.md`: copy-weighted card frequency over the 4,523 corpus lists, top-N what-if, tail shape). Then: deploy rc2 to build-api (DEPLOY §5) if DEPLOYABLE; catalogue coverage program sized from the report; control finisher schedule stage (`…-stage4-…md`); freeze 1.3.0. Standard total needs the §7 interaction/advantage norms revisited — operator's five spec questions still unanswered.
