...
### 2026-09-21 — Deck Score v1.3 stages 1–4c committed; refuter: NOT DEPLOYABLE (coverage); coverage round 1 running
...
- **Open:** 61b8b66 stage 4c (saturation per profile: Commander `.70` = p80 kept, `Q_SATURATION_BRAWL = .712`; `Q_BASELINE_CLOSING_BRAWL`; four atoms; `SCORE_VERSION 1.3.0-rc2`; anchors 15/16, tazri-upgraded 20 needs a band ruling, not a floor move). f5dd168 coverage sizing (`scripts/deck-score-coverage-corpus.ts`, `coverage-queue-corpus.csv`): real lists typed-coverage p50 33 % / 37 %, 0.7 % / 0.1 % ≥ 80 %; the queue head is staples the generator was never run over (Chaos Warp, Skullclamp, Go for the Throat…), top-5,000 = 4,279 unknown / 721 partial, tail atom-shaped. **Opus refuter (`v13-refute-report.md`): NOT DEPLOYABLE** — R1 through the shipped `scoreDeckSafely` 193/199 held-out real Commander lists score ≤ 20 (S = 0 on 187), Brawl 200/200, i.e. the pile criteria carry no discrimination and the 4c spread numbers were `--raw` (gate lifted); R2 three stage-4b constants stale after the 4c atoms (closing Brawl .338 → .333, counters band 10/14 → 9/12) and the pins are artefact copies; R3 fixture commanders are IN the band training strides (50/1,838, 30/1,166 — `HELD_OUT_COMMANDERS` only guards pile generation); R4 2/30 cEDH Top-16 lists score 20 with S = 0. Everything else reproduced (floors .574/.675/.323, anchors bit-identical, piles, cEDH 88/90, parser 5,244 lookups 0 diffs, wiring untouched, 6/6 probes). Running: coverage round 1 (`deck-score-coverage-round1-2026-09-21.md`, Opus): generate over the corpus, atoms for the partial tail, fix R3 in `strideOrder`, `bands verify` gate that re-measures every frozen constant, diagnose R4, re-freeze everything last; acceptance real-list S = 0 ≤ 30 %, coverage p50 ≥ 70/75 %. Then refuter again → deploy → control finisher schedule (`…-stage4-…md`) → freeze 1.3.0. Build-api stays on d6e0e2d. Standard total needs the §7 norms revisited — operator's five spec questions still unanswered.
